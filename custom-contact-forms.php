...
<div id="customcontactforms-admin">
  ...
  <ul id="plugin-nav">
    ...
    <li><a href="#form-submissions">Saved Form Submissions</a></li>
    ...
    <tbody>
      ...
      <tr<?php if ($z % 2 == 1) echo ' class="evenrow"'; ?>>
          ...
          <input type="hidden" class="object-id" name="fid" value="<?php echo $fields[$i]->id; ?>" />
            ...
            <input type="submit" class="edit-button" name="field_edit" value="Edit" />
    ...
    <tbody>
      ...
      <tr <?php if ($z % 2 == 0) echo ' class="evenrow"'; ?>>
          ...
          <input type="hidden" class="object-id" name="fid" value="<?php echo $fields[$i]->id; ?>" />
            ...
            <input type="submit" name="field_edit" class="edit-button" value="Edit" /></td>
      </tr>
      <tr <?php if ($z % 2 == 0) echo ' class="evenrow"'; ?>>
      	<td class="fixed-fields-extra-options" colspan="7">Field Instructions: <input type="text" name="field[field_instructions]" class="width200" value="<?php echo $fields[$i]->field_instructions; ?>" /> - <?php echo $this->fixed_fields[$fields[$i]->field_slug]; ?></td>
      </tr>
      </form>
      <?php
                }
    	...
    	<table id="edit-field-options">
			...
			foreach ($options as $option) {
				...
				<tr<?php if ($i % 2 == 1) echo ' class="evenrow-field-options"'; ?>>
					...
					<td class="action">
                    	<input type="submit" value="Edit" class="edit-button" name="edit_field_option" /> 
    ...
    <tbody>
      ...
      <tr class="<?php if ($i % 2 == 0) echo 'evenrow'; ?>">
          ...
          <td><input type="hidden" class="object-id" name="fid" value="<?php echo $forms[$i]->id; ?>" />
            ...
            <input type="submit" name="form_edit" class="edit-button" value="Edit" />
  ...
  <table class="widefat post" id="form-submissions" cellspacing="0">
	...
	foreach ($user_data_array as $data_object) {
		$data = new CustomContactFormsUserData(array('form_id' => $data_object->data_formid, 'data_time' => $data_object->data_time, 'encoded_data' => $data_object->data_value));	
	?>
  ...
  <table class="widefat post" id="manage-styles" cellspacing="0">
	...
	foreach ($styles as $style) {
        ...
        <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
            ...
            <input type="submit" class="submit-styles edit-button" name="style_edit" value="Update Style" /><br />
		...
		function processForms() {
			if ($_POST[ccf_customhtml]) {
				...
				if (empty($errors)) {
					require_once('custom-contact-forms-user-data.php');
					require_once('custom-contact-forms-mailer.php');
					$data_object = new CustomContactFormsUserData(array('data_array' => $data_array, 'form_page' => $_SERVER['REQUEST_URI'], 'form_id' => 0, 'data_time' => time()));
					...
					if ($admin_options[email_form_submission] == 1) {
						...
						//echo "Location: " . $_POST[thank_you_page];
				...
				if (empty($errors)) {
					...
					$data_object = new CustomContactFormsUserData(array('data_array' => $data_array, 'form_page' => $_POST[form_page], 'form_id' => $form->id, 'data_time' => time()));
					parent::insertUserData($data_object);
					$body .= 'Sender IP: ' . $_SERVER['REMOTE_ADDR'] . "\n";
