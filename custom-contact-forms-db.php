...
if (!class_exists('CustomContactFormsDB')) {
	class CustomContactFormsDB {
		...
		function CustomContactFormsDB() {
			...
			$this->user_data_table = $table_prefix . 'customcontactforms_user_data';
		...
		function createTables() {
			...
			if(!$this->formsTableExists()) {
				...
				$sql7 = " CREATE TABLE `".$this->user_data_table."` (
						`id` INT( 11 ) NOT NULL AUTO_INCREMENT ,
		...
		function userDataTableExists() {
			global $wpdb;
			return ($wpdb->get_var("show tables like '". $this->user_data_table . "'") == $this->user_data_table);
		}
		...
		function deleteUserData($uid) {
			global $wpdb;
			$wpdb->query("DELETE FROM " . $this->user_data_table . " WHERE id='$uid'");
			return true;
		}
		...
		function selectAllUserData() {
			global $wpdb;
			return $wpdb->get_results("SELECT * FROM " . $this->user_data_table . " ORDER BY data_time DESC");	
		}
		...
		function selectUserData($uid) {
			global $wpdb;
			return $wpdb->get_row("SELECT * FROM " . $this->user_data_table . " WHERE id='$uid'");
		}
		...
		function insertUserData($data_object) {
			...
			$wpdb->insert($this->user_data_table, array('data_time' => $data_object->getDataTime(), 'data_formid' => $data_object->getFormID(), 'data_formpage' => $data_object->getFormPage(), 'data_value' => $data_object->getEncodedData()));
			return $wpdb->insert_id;
		}
