...
$j(document).ready(function() {
			...
			$j.ajax({
				...
				success: function(data) {
					...
					if (object_type == "style") {
						...
						var fields_options_input = $j("select[name=attach_object_id], select[name=detach_object_id]");
	...
	$j(".attach-button").live("click", function() {
		...
		var detach_object_field = $j(this).parents().find("select[name=detach_object_id]:first");
	...
	$j(".detach-button").each(function(){
		var name = $j(this).attr('name');
		var value = $j(this).attr('value');
		var html = '<input class="detach-button" type="button" name="' + name + '" value="' + value + '" />';
		$j(this).after(html).remove(); // add new, then remove original input
	});
	$j(".detach-button").live("click", function() {
		...
		if (detach_object_id != "-1") {
			...
			var save_box = fx.initSaveBox(detaching);
			$j.ajax({
				type: "POST",
				url: ccf_file,
				data: "ajax_action=detach&detach_object_id=" + detach_object_id + "&detach_from=" + object_id + "&object_type=" + object_type,
				success: function(data) {
					//debug = fx.initDebugWindow();
					//$j("<div></div>").html(data).appendTo(debug);
